...
**Base URL**:  
- **Development**: `http://localhost:8000` (or your Replit dev URL)
- **Production**: `https://eattendanceapi.anglosingapore.ac.th` (configured for Anglo Singapore deployment)

**Database**: MySQL (srv1521.hstgr.io)  
**Model**: InsightFace buffalo_l or antelopev2  
...
The FastAPI server should be configured to run on port 8000 and bind to all interfaces:
...
The API must enable CORS to allow requests from the Node.js attendance system frontend. Configure CORS to allow:
...
```bash
DB_HOST=srv1521.hstgr.io
DB_NAME=u311154254_TestAttendance
DB_USER=u311154254_TADB
DB_PASSWORD=Anglo!123456
```
...
CREATE TABLE IF NOT EXISTS FaceData (
    ...
    UNIQUE KEY unique_person (PersonType, PersonCode)
);
...
### Integration with Node.js Attendance System

The Node.js attendance system (running on port 5000) proxies requests to this InsightFace API through the following endpoints:

- `POST /api/insightface/enroll` → proxies to InsightFace `/api/face/enroll`
- `POST /api/insightface/recognize` → proxies to InsightFace `/api/face/recognize`
- `GET /api/insightface/list/:personType` → proxies to InsightFace `/api/face/list/{personType}`
- `DELETE /api/insightface/delete` → proxies to InsightFace `/api/face/delete`

The Node.js system checks API health on startup via `/api/health` endpoint.

### Replit Deployment

When deploying on Replit:

1. Set the run command to: `uvicorn main:app --host 0.0.0.0 --port 8000`
2. Configure secrets for database credentials
3. Ensure ONNX models (buffalo_l or antelopev2) are included in the project
4. Set the deployed URL in the Node.js system's `INSIGHTFACE_API_URL` variable

### Security Recommendations

- **Add API Key Authentication**: Implement API key validation for production
- **Rate Limiting**: Add rate limiting to prevent abuse
- **Input Validation**: Validate all input parameters and file uploads
- **HTTPS Only**: Always use HTTPS in production
- **Database Security**: Use separate read-only credentials where possible
- **Audit Logging**: Enable comprehensive logging for security monitoring

### Troubleshooting

**Issue**: "Face recognition engine not initialized"
- **Solution**: Ensure ONNX model files are present and properly loaded

**Issue**: "Database connection failed"
- **Solution**: Verify database credentials and network connectivity

**Issue**: "No faces detected in the image"
- **Solution**: Ensure image quality is good, face is clearly visible, and proper lighting

**Issue**: CORS errors from frontend
- **Solution**: Verify CORS middleware is properly configured with correct origins